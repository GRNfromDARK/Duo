...
| `src/types/god-schemas.ts` | God LLM 任务分析 schema（TaskType + GodTaskAnalysis） | |
...
| `phases` | `Phase[] \| null \| undefined` | 子阶段列表（compound 类型必填，其他类型可省略或为 null） |
...
```
           ...
           └─────────────────────┘     │  StopRoleSchema           │
                                       │  RetryRoleSchema          │
              god-envelope.ts          │  SwitchAdapterSchema      │
           ┌─────────────────────┐     │  SetPhaseSchema           │
           │  DiagnosisSchema     │     │  AcceptTaskSchema         │
           │  AuthoritySchema     │     │  WaitSchema               │
           │  EnvelopeMessageSchema│     │  RequestUserInputSchema   │
           │  AutonomousResolution│     │  ResumeAfterInterruptSchema│
           │  GodDecisionEnvelope─┼──(actions)──► GodActionSchema   │
           └─────────────────────┘     └──────────────────────────┘

              observation.ts                  degradation.ts
           ┌─────────────────────┐     ┌─────────────────────────┐
           │  ObservationType (13种) │     │  GodErrorKind           │
           │  ObservationSource   │     └─────────────────────────┘
           │  ObservationSeverity │
           │  Observation         │
           │  isWorkObservation() │
