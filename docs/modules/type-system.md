...
Duo 的类型系统分布在 9 个文件中，定义了从 CLI adapter 到 God 决策的完整类型体系。
...
| `src/types/god-actions.ts` | Hand / GodAction 结构化动作目录 | |
| `src/types/god-envelope.ts` | GodDecisionEnvelope 统一决策信封 + Authority 类型 | |
| `src/types/observation.ts` | Observation 归一化类型系统 | |
| `src/types/degradation.ts` | 降级状态与错误类型 | |
...
### `AutonomousResolution` — 自主决议

God 代理决策的记录结构（来源：BUG-24 修复）。当 God 代替用户做出选择时，记录完整的决策过程以供审计。

| 字段 | 类型 | 说明 |
|------|------|------|
| `question` | `string` | 待决策的问题 |
| `choice` | `string` | 初始选择 |
| `reflection` | `string` | 对选择的反思 |
| `finalChoice` | `string` | 反思后的最终选择 |
...
| `authority` | `Authority` | 是 | 权限声明 |
| `actions` | `GodAction[]` | 是 | 要执行的动作列表（来自 `god-actions.ts`） |
| `messages` | `EnvelopeMessage[]` | 是 | 附带的消息列表 |
| `autonomousResolutions` | `AutonomousResolution[]` | 否 | God 代理决策记录（BUG-24） |
...
13 种观测类型的 `as const` 数组，作为 `ObservationTypeSchema` 的基础。
...
## degradation.ts — 降级状态类型

> 从 `degradation-manager.ts` 中提取的共享类型，被 WatchdogService 和 session 持久化共同使用。与已废弃的 `DegradationManager` 类解耦。
...
| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| `level` | `DegradationLevel` | 是 | 当前降级等级 |
| `consecutiveFailures` | `number` | 是 | 连续失败次数 |
| `godDisabled` | `boolean` | 是 | God 是否已被禁用 |
| `fallbackActive` | `boolean` | 是 | 降级回退策略是否已激活 |
| `lastError` | `string` | 否 | 最后一次错误的描述 |

### `DegradationNotification`

降级事件的通知消息，用于向上层传递降级状态变化。

| 字段 | 类型 | 说明 |
|------|------|------|
| `type` | `'retrying' \| 'fallback_activated' \| 'god_disabled'` | 通知类型 |
...
| 值 | 含义 |
|----|------|
| `retrying` | 正在重试 God 调用 |
| `fallback_activated` | 降级回退策略已激活 |
| `god_disabled` | God 已被完全禁用 |
...
```
           ...
           └─────────────────────────┘     │  AcceptTaskSchema         │
                                           │  WaitSchema               │
              god-envelope.ts              │  RequestUserInputSchema   │
           ┌─────────────────────────┐     │  ResumeAfterInterruptSchema│
           │  DiagnosisSchema         │     │  EmitSummarySchema        │
           │  AuthoritySchema         │     │  GodActionSchema ◄────────┼─┐
           ...
           ┌─────────────────────────┐     ┌─────────────────────────┐
           │  ObservationType (13种)  │     │  DegradationLevel       │
           │  ObservationSource       │     │  GodErrorKind           │
           │  ObservationSeverity     │     │  DegradationState       │
           │  Observation             │     │  DegradationNotification│
           │  isWorkObservation()     │     └─────────────────────────┘
           └─────────────────────────┘
```
...
5. **God 决策**：`Observation` → God LLM → `GodDecisionEnvelope`（含 `Diagnosis` + `Authority` + `GodAction[]` + `EnvelopeMessage[]` + 可选 `AutonomousResolution[]`）
6. **动作执行**：`GodAction[]` → Hand 逐一执行 → 状态变更（发送消息、切换阶段、接受任务等）
7. **展示阶段**：`Message.role`（`RoleName`）→ `getRoleStyle()` → `RoleStyle` 视觉样式；`ScrollState` 管理滚动
8. **降级管理**：God 错误（`GodErrorKind`）→ `DegradationState` 级别提升 → `DegradationNotification` 通知上层

### 架构演进说明

类型系统从 5 文件（v1）演进到 9 文件（v2），核心变化为引入 Sovereign God Runtime 架构：
...
| 新增 `degradation.ts` | 从 DegradationManager 中提取共享类型，支持 WatchdogService 和 session 持久化 |
