...
|------|-------------|----------------------|
| 决策入口 | routePostCoder / routePostReviewer / evaluateConvergence / classifyTask 等散点调用 | `GodDecisionService.makeDecision()` 单入口 |
...
| `GodAutoDecisionSchema` | 自主决策 | deprecated（已删除 auto-decision.ts） |
...
### 10.1 `src/god/watchdog.ts` — Watchdog AI 故障诊断
...
### 10.2 `src/god/degradation-manager.ts` — 四级降级管理
