...
| `useRenderer()` | 访问底层 `CliRenderer` 实例，用于检查 selection 状态和通过 OSC52 写入剪贴板。支持 Ctrl+C / Cmd+C（`key.super`）/ Option+C（`key.meta`）复制选中文本；App.tsx 通过 `renderer.on('selection', ...)` 事件在拖选结束时缓存文本和 Selection 对象引用（identity-based cache），防止流式渲染导致的 stale selection |
...
| 快捷键 | 功能 |
|--------|------|
| `Ctrl+C` | 有选区时复制（OSC52）；无选区时中断 LLM / 退出（双击） |
| `Cmd+C` (macOS) | 有选区时复制（OSC52）；无选区时忽略 |
| `Ctrl+N` | 新会话 |
