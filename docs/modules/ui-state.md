...
| 函数 | 输入 | 输出 | 关键逻辑 |
|------|------|------|----------|
| `createEscapeWindowState(decision)` | `GodAutoDecision` | `EscapeWindowState` | 创建即 `confirmed: true, visible: false, countdown: 0`，`decisionPreview: "[action] reasoning"` |
| `handleEscapeKey(state, key)` | 当前状态 + `'escape'`/`'space'` | `EscapeWindowState` | 已 confirmed/cancelled 时返回原状态引用；escape -> cancelled + visible:false；space -> confirmed + visible:false |
| `tickEscapeCountdown(state)` | 当前状态 | `EscapeWindowState` | 已 resolved（cancelled 或 confirmed）时返回原状态；countdown 减 1；减至 0 时 confirmed |
...
| 函数 | 输入 | 输出 | 关键逻辑 |
|------|------|------|----------|
| `createGodOverlayState(analysis, auditEntries, convergenceLog)` | `GodTaskAnalysis` + audit 日志 + convergence 日志 | `GodOverlayState` | `visible: false`；从末尾倒序搜索最新 `POST_REVIEWER` audit entry 提取 `confidenceScore`；compound 任务从最新 `PHASE_TRANSITION` entry 提取 `currentPhase`（fallback 到 `analysis.phases[0].id`） |
| `handleGodOverlayKey(state, key)` | 当前状态 + 按键字符串 | `{ state, action? }` | escape -> 关闭（`visible: false`）；`r`/`s`/`f`/`p`（不区分大小写，使用 `toLowerCase()`） -> 返回对应 action |
| `writeGodOverlayActionAudit(sessionDir, opts)` | session 目录 + `{ seq, round, action, taskType, phase? }` | `void` | 将手动干预事件写入 audit log，`decisionType: 'MANUAL_INTERVENTION'`，`inputSummary` 包含操作描述和上下文，`decision` 记录 actionType/taskType/phase |
