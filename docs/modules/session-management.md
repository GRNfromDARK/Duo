...
> 源文件: `src/session/session-starter.ts` | `src/session/session-manager.ts` | `src/session/prompt-log.ts`
>
...
会话管理模块负责 Duo 会话的完整生命周期：**创建与校验** (`session-starter`) -> **持久化与恢复** (`session-manager`) -> **Prompt 审计日志** (`prompt-log`)。

三个子模块协作确保：
- 会话状态在进程崩溃后仍可安全恢复
- 所有发送给 LLM 的 prompt 都被完整记录，用于调试与审计

> **重构说明**：原 `context-manager.ts`（负责 Prompt 构建与 token 预算控制）已在 "God Authoritative" 重构中删除。Prompt 构建职责现由各调用方（God Decision Service、Adapter 层）直接承担。
...
| `godTaskAnalysis` | `GodTaskAnalysis?` | God 任务分析结果（仅首次分析时写入，FR-011） |
...
6. 初始状态：`status='created', currentRole='coder'`
