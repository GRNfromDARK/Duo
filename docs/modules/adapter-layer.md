...
- 输出缓冲区上限 50MB（`DEFAULT_MAX_BUFFER_BYTES`），超出时丢弃旧数据只保留最新的 50MB
...
3. 始终触发 `process-complete` 事件——适配器据此关闭 ReadableStream 的 controller
...
1. 清除定时器（timeout、heartbeat）
2. 移除 `process.on('exit')` 处理器，防止内存泄漏
3. 若进程仍在运行，先调用 `kill()` 等待其终止
...
| 机制 | 默认值 | 行为 |
|------|--------|------|
| 全局超时 | 10 分钟（`DEFAULT_TIMEOUT_MS`） | 超时后触发 `timeout` 事件并自动调用 `kill()` |
...
- **会话管理**：从 `thread.started` 事件中捕获 `thread_id`，逻辑与 Claude Code 类似
...
2. **实现**：创建 `src/adapters/<name>/adapter.ts`，实现 `CLIAdapter` 接口
   ...
   - 监听 ProcessManager 的 `process-error` 和 `process-complete` 事件来驱动 ReadableStream 的 error/close
