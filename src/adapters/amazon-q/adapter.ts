/**
 * AmazonQAdapter — Amazon Q CLI adapter implementation.
 * Source: FR-011b
 *
 * Key behaviors:
 * - Calls: q chat --no-interactive --trust-all-tools <prompt>
 * - Uses TextStreamParser for plain text output parsing
 * - detectCommand: q version (no --version flag)
 */
...
export class AmazonQAdapter implements CLIAdapter {
  readonly name = 'amazon-q';
  readonly displayName = 'Amazon Q';
  ...
  async isInstalled(): Promise<boolean> {
    return new Promise((resolve) => {
      execFile('q', ['version'], { timeout: 5000 }, (err) => {
        resolve(!err);
      });
    });
  }
  ...
  async getVersion(): Promise<string> {
    return new Promise((resolve) => {
      execFile('q', ['version'], { timeout: 5000 }, (err, stdout) => {
        if (err) {
          resolve('unknown');
          return;
        }
  ...
  /**
   * Build CLI arguments for q command.
   * Exposed for testing.
   */
  ...
  async *execute(
    ...
    const { env, replaceEnv } = buildAdapterEnv({
      requiredPrefixes: ['AWS_', 'AMAZON_'],
      extraEnv: opts.env,
    });

    const execOpts: ExecOptions = { ...opts, env, replaceEnv };

    const child = this.processManager.spawn('q', args, execOpts);
