/**
 * QwenAdapter — Qwen CLI adapter implementation.
 * Source: FR-011b
 *
 * Key behaviors:
 * - Calls: qwen -p <prompt> --output-format stream-json
 * - Uses StreamJsonParser (shared with Claude Code, Gemini, Amp)
 */
...
export class QwenAdapter implements CLIAdapter {
  ...
  async isInstalled(): Promise<boolean> {
    return new Promise((resolve) => {
      execFile('qwen', ['--version'], { timeout: 5000 }, (err) => {
        resolve(!err);
      });
    });
  }
  ...
  async getVersion(): Promise<string> {
    return new Promise((resolve) => {
      execFile('qwen', ['--version'], { timeout: 5000 }, (err, stdout) => {
        if (err) {
          resolve('unknown');
          return;
        }
  ...
  /**
   * Build CLI arguments for qwen command.
   * Exposed for testing.
   */
  ...
  async *execute(
    ...
    const { env, replaceEnv } = buildAdapterEnv({
      requiredPrefixes: ['QWEN_', 'DASHSCOPE_'],
      extraEnv: opts.env,
    });

    const execOpts: ExecOptions = { ...opts, env, replaceEnv };

    const child = this.processManager.spawn('qwen', args, execOpts);
