/**
 * ClineAdapter — Cline CLI adapter implementation.
 * Source: FR-011b
 *
 * Key behaviors:
 * - Calls: cline -y <prompt> --json
 * - Uses JsonlParser for JSON output parsing
 * - -y flag serves as both exec command and yolo flag
 */
...
export class ClineAdapter implements CLIAdapter {
  readonly name = 'cline';
  ...
  async isInstalled(): Promise<boolean> {
    return new Promise((resolve) => {
      execFile('cline', ['--version'], { timeout: 5000 }, (err) => {
        resolve(!err);
      });
    });
  }
  ...
  async getVersion(): Promise<string> {
    return new Promise((resolve) => {
      execFile('cline', ['--version'], { timeout: 5000 }, (err, stdout) => {
        if (err) {
          resolve('unknown');
          return;
        }
  ...
  /**
   * Build CLI arguments for cline command.
   * Exposed for testing.
   */
  buildArgs(prompt: string, opts: ExecOptions): string[] {
    const args: string[] = ['-y', prompt];
  ...
  async *execute(
    ...
    const { env, replaceEnv } = buildAdapterEnv({
      requiredPrefixes: ['OPENAI_', 'ANTHROPIC_', 'CLINE_'],
      extraEnv: opts.env,
    });

    const execOpts: ExecOptions = { ...opts, env, replaceEnv };

    const child = this.processManager.spawn('cline', args, execOpts);
