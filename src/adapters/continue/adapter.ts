/**
 * ContinueAdapter — Continue CLI adapter implementation.
 * Source: FR-011b
 *
 * Key behaviors:
 * - Calls: cn -p <prompt> --format json --allow
...
export class ContinueAdapter implements CLIAdapter {
  ...
  async isInstalled(): Promise<boolean> {
    return new Promise((resolve) => {
      execFile('cn', ['--version'], { timeout: 5000 }, (err) => {
        resolve(!err);
      });
    });
  }
  ...
  async getVersion(): Promise<string> {
    return new Promise((resolve) => {
      execFile('cn', ['--version'], { timeout: 5000 }, (err, stdout) => {
        if (err) {
          resolve('unknown');
          return;
        }
  ...
  /**
   * Build CLI arguments for cn command.
   * Exposed for testing.
   */
  buildArgs(prompt: string, opts: ExecOptions): string[] {
    const args: string[] = ['-p', prompt, '--format', 'json'];
  ...
  async *execute(
    ...
    const { env, replaceEnv } = buildAdapterEnv({
      requiredPrefixes: ['CONTINUE_'],
      extraEnv: opts.env,
    });

    const execOpts: ExecOptions = { ...opts, env, replaceEnv };

    const child = this.processManager.spawn('cn', args, execOpts);
