/**
 * AiderAdapter — Aider CLI adapter implementation.
 * Source: FR-011b
 *
 * Key behaviors:
 * - Calls: aider -m <prompt> --yes-always
...
export class AiderAdapter implements CLIAdapter {
  ...
  async isInstalled(): Promise<boolean> {
    return new Promise((resolve) => {
      execFile('aider', ['--version'], { timeout: 5000 }, (err) => {
        resolve(!err);
      });
    });
  }
  ...
  async getVersion(): Promise<string> {
    return new Promise((resolve) => {
      execFile('aider', ['--version'], { timeout: 5000 }, (err, stdout) => {
        if (err) {
          resolve('unknown');
          return;
        }
  ...
  /**
   * Build CLI arguments for aider command.
   * Exposed for testing.
   */
  buildArgs(prompt: string, opts: ExecOptions): string[] {
    const args: string[] = ['-m', prompt];
  ...
  async *execute(
    ...
    const { env, replaceEnv } = buildAdapterEnv({
      requiredPrefixes: ['OPENAI_', 'ANTHROPIC_', 'AIDER_'],
      extraEnv: opts.env,
    });

    const execOpts: ExecOptions = { ...opts, env, replaceEnv };

    const child = this.processManager.spawn('aider', args, execOpts);
