/**
 * CopilotAdapter — GitHub Copilot CLI adapter implementation.
 * Source: FR-011b
 *
 * Key behaviors:
 * - Calls: copilot -p <prompt> --allow-all-tools
...
export class CopilotAdapter implements CLIAdapter {
  ...
  async isInstalled(): Promise<boolean> {
    return new Promise((resolve) => {
      execFile('copilot', ['--version'], { timeout: 5000 }, (err) => {
        resolve(!err);
      });
    });
  }
  ...
  /**
   * Build CLI arguments for copilot command.
   * Exposed for testing.
   */
  ...
  async *execute(
    ...
    const { env, replaceEnv } = buildAdapterEnv({
      requiredPrefixes: ['GITHUB_', 'COPILOT_'],
      extraEnv: opts.env,
    });

    const execOpts: ExecOptions = { ...opts, env, replaceEnv };

    const child = this.processManager.spawn('copilot', args, execOpts);
